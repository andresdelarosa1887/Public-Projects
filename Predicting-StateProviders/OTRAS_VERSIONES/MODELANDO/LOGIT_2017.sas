...
y se toma la tabla logit.337; 

*LOS ANOS ADJUDICADOS PARA CADA RPE ESTAN AJUSTADOS A QUE SEAN EL ANO 2017; 
*ADJUDICACIONES DEL ANO 2017 COMO VARIABLE DEPENDIENTE;
*ods select none; 

proc freq data=rpe.logit_337; 
tables adjudicado_2017*genero; 
run; 
...
proc logistic data=rpe.logit_337 PLOTS(MAXPOINTS=NONE)
	plots(only)=(effect oddsratio);
	class genero (param=reference ref='Masculino') 
	clasificacion_empresarial_3;
	model ADJUDICADO_2017(event='1')= Genero clasificacion_empresarial_3
	ADJANO_2016	DISTANOS_ADJ; 
	TITLE1 "QUE CARACTERISTICAS DE LOS PROVEEDORS INFLUYERON A QUE ESTOS FUERAN ADJUDICADOS"; 
	TITLE2 "EN EL 2017";
	store out=logit_2017; 
run; 
